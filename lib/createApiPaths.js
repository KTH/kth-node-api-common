  ...
  Object.keys(swaggerPaths).forEach(pathname => {
    ...
    Object.keys(config).forEach(verb => {
      ...
      if (operation.security) {
        try {
          ensureValidSecurityArray(operation.security)
        } catch (error) {
          error.message = `Problem in createApiPaths() - Invalid structure of "security" (${pathname}.${verb}) - ${error.message}`
          throw error
        }

        operation.security.forEach(item => {
          if (item.api_key) {
            newEndpoint.openid = {
              scopes: item,
              scope_required: true,
      ...
      allPaths[operation.operationId] = newEndpoint
    })
  })
